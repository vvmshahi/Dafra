...
assert.match(pos, /afterSaleAction: 'receipt' \| 'a4' \| 'both'/)
assert.match(pos, /afterSaleAction=\{resolvedInvoiceSettings\.afterSaleAction\}/)
assert.doesNotMatch(pos, /printMode=\{branch\?\.print_mode/)
assert.match(detail, /documentFromStoredInvoice/)
assert.ok(detail.indexOf('const documentViewModel = useMemo') < detail.indexOf('if (loading)'), 'Invoice Detail model hook must run before loading return')
assert.match(detail, /if \(!invoice \|\| !branch\) return null/)
assert.match(receipt, /documentFromStoredInvoice/)
for (const page of [pos, detail, receipt]) assert.match(page, /<ThermalReceipt model=\{documentViewModel\}/)
