...
for (const marker of ['ContemporaryItemName', 'ContemporaryItemTable', 'contemporaryColumnLabel', 'a4-contemporary-head', 'a4-contemporary-closeout']) assert.match(source, new RegExp(marker))
...
assert.match(css, /\.a4-contemporary-brand \{ display: grid; align-self: center; min-width: 0; padding-bottom: \.8mm/)
assert.match(css, /\.a4-contemporary-brand \.a4-display-heading \{[^}]*font-size: 16\.5pt/)
assert.match(css, /\.a4-contemporary-qr \{ display: grid; place-items: center; align-self: start; padding: \.7mm/)
assert.match(css, /\.a4-contemporary-qr \.a4-qr \{ display: grid; justify-items: center; gap: \.25mm/)
assert.match(css, /\.a4-contemporary-qr \.a4-qr img,.a4-contemporary-qr \.a4-qr-placeholder \{ width: 21mm; height: 21mm/)
...
try {
  ...
  assert.equal(registry.A4_TEMPLATE_REGISTRY.contemporary_border.qrRegion, 'upper-identity-region')
  ...
  for (const value of ['a4-contemporary-head', 'a4-contemporary-brand', 'a4-contemporary-document', 'a4-contemporary-qr', 'a4-contemporary-parties', 'a4-contemporary-items', 'a4-contemporary-closeout', 'Trading House', 'Legal Seller Co', '300000000000003']) assert.match(preview, new RegExp(value))
  ...
  assert.ok(preview.indexOf('class="a4-contemporary-qr"') < preview.indexOf('class="a4-contemporary-parties"'), 'QR remains near document metadata')
  ...
  assert.match(noLogo, /a4-contemporary-head--brandless/); assert.match(noLogo, /a4-contemporary-document--without-qr/); assert.doesNotMatch(noLogo, /class="a4-logo|class="a4-qr"/)
  ...
  assert.match(walkIn, /a4-contemporary-parties a4-parties--seller-only/); assert.doesNotMatch(walkIn, /class="a4-buyer"/)
  ...
  for (const value of ['SAMPLE-CN-0042', 'SAMPLE-0042', 'Sample return', 'a4-contemporary-items']) assert.match(creditMarkup, new RegExp(value))
} finally { await server.close() }

console.log('Contemporary Modular A4 regression matrix passed (branded header, metadata QR, seller/buyer, canonical items, closeout, long documents, credit notes, tokens, and preview parity).')
