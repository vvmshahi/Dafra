...
check(!restockEligibility({ returnStock: true, branchStockEnabled: true, source: 'custom', productId: null, stockTrackedAtSale: false, serviceAtSale: false }).restock, 'Custom Line credit needs no product and never restocks')
...
const permittedPaths = new Set([
  ...
  // Explicit Android print reliability exception: validates already-issued
  // document rendering and printer dispatch only. It cannot change checkout,
  // reporting, credit authority, migrations, or ZATCA submission.
  ...
  // Authorized Contemporary Modular A4 presentation coverage only; it reads
  // the canonical issued-document model and cannot change financial authority.
  ...
  'scripts/test-custom-line-persistence.mjs',
  'package.json',
