...
assert.match(importPanel, /\.from\('products'\)\.select\('sku,branch_id,branches\(name,name_ar\)'\)\.eq\('tenant_id', tenantId\)\.in\('sku', duplicateSkus\)/)
assert.match(catalogueDialog, /<CatalogueImportPanel key=\{branchId\} branchId=\{branchId\} branchName=\{branchName\} tenantId=\{tenantId\}/)
