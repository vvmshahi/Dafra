...
matches(migration, /NEW\.service_item_at_sale := true;/, 'provenance trigger must mark custom snapshots as no-stock service semantics')
matches(migration, /'product_id', NULL,[\s\S]*'product_unit_id', NULL/, 'custom item rows must carry no product or product unit')
matches(migration, /'stock_tracked_at_sale', false,[\s\S]*'service_item_at_sale', true/, 'custom item rows must not enter stock handling')
