...
assert.match(page, /allTabs\.filter\(tab => tab\.id !== 'printerSetup' \|\| electron\)/)
assert.match(page, /electron && active === 'printerSetup'/)
assert.doesNotMatch(page, /electronAPI/)
assert.match(printerTab, /if \(!isElectron\(\)\)/)
assert.match(electron, /window\.electronAPI\?\.isElectron === true/)
