...
for (const tab of ['general', 'access', 'billing', 'pos', 'modules', 'invoices']) {
...
assert.match(source, /firstInvalidField\?\.startsWith\('login_'\)/)
assert.match(source, /firstInvalidField === 'business_profile'/)
...
assert.match(source, /isLegacyServiceBranch && option\.key === 'enabled'/)
assert.match(source, /canEditBillingConfig \? \[\{ id: 'billing'/)
assert.match(source, /useBranchBillingConfig\(branch\?\.id\)/)
assert.match(source, /updateBranchBillingProfileConfig/)
