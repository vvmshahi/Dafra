...
assert.match(tradingReconnect, /statusLabel\)\)\|failed|\['not_started'.*'failed'\]/)
assert.match(tradingReconnect, /if \(next\?\.status === 'failed'\) next = null/)
...
assert.match(onboarding, /SANDBOX_CSR_GENERATION_FAILED/)
assert.match(onboarding, /SANDBOX_CSR_KEY_MISMATCH/)
assert.match(onboarding, /SANDBOX_CREDENTIAL_PERSIST_FAILED/)
assert.match(onboarding, /SANDBOX_ACTIVATION_FAILED/)
assert.match(onboarding, /SANDBOX_UPSTREAM_UNAVAILABLE/)
for (const stage of [
  'authorize_caller', 'resolve_trading_scope', 'generate_private_key', 'generate_csr',
  'validate_csr_key_match', 'compliance_csid_request', 'compliance_csid_response',
  'compliance_validation', 'production_csid_request', 'certificate_key_match',
  'credential_persist', 'activate_sandbox_profile',
]) assert.match(onboarding, new RegExp(`'${stage}'`))
assert.match(onboarding, /requestId/)
assert.match(onboarding, /upstreamStatus/)
assert.match(onboarding, /assertFreshIdentityMatches/)
assert.match(onboarding, /requestProductionCsid/)
assert.match(onboarding, /\.in\('status', \[\.\.\.CURRENT_TRADING_SANDBOX_STATUSES\]\)/)
assert.match(onboarding, /\.in\('onboarding_status', \[\.\.\.RESUMABLE_TRADING_SANDBOX_STATES\]\)/)
assert.match(onboarding, /body\.otp/)
assert.match(onboarding, /csrPem: credential\.csr_pem,\s+otp,\s+onResponse/)
...
assert.match(onboarding, /resolve_zatca_trading_sandbox_scope/)
