...
for (const component of ['CompactRetailReceipt', 'StructuredDetailReceipt', 'BrandedModernReceipt']) assert.match(thermalCompositions, new RegExp(`export function ${component}`))
for (const marker of ['thermal-compact-line', 'thermal-structured-line__figures', 'thermal-branded-card__head']) assert.match(thermalCompositions, new RegExp(marker))
...
for (const thumbnail of ['receipt-theme-thumb--compact', 'receipt-theme-thumb--standard', 'receipt-theme-thumb--detailed']) assert.match(css, new RegExp(thumbnail))
