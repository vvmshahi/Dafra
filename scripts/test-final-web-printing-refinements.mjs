...
const androidPrintExecution = read('src/lib/print/androidPrintExecution.ts')
const receiptPrint = read('src/lib/receiptPrint.ts')
const indexHtml = read('index.html')
const documentLocalization = read('src/localization/documents.ts')
...
assert.match(indexHtml, /family=Cairo:wght@400;500;600;700/)
assert.match(documentLocalization, /'Cairo', 'KubriArabic'/)
assert.match(receiptPrint, /createElement\('iframe'\)/)
assert.match(receiptPrint, /params\.set\('embedded', '1'\)/)
assert.match(receiptPrint, /contentWindow\?\.print\(\)/)
assert.match(receiptPrint, /function openBrowserReceiptPrint\(invoiceId: string\)[\s\S]*openPrintPopup\(receiptPreviewUrl\(invoiceId, true\)\)/)
assert.match(receiptPrint, /isAndroidBrowser\(\)[\s\S]*openBrowserReceiptPrint/)
assert.match(posPage, /executeAndroidPrint\([\s\S]*printCurrentPageDocument\('pos-pdf-printable', 'invoice', validate\)/)
assert.doesNotMatch(posPage, /isAndroidBrowser\(\)[\s\S]*openPrintPopup/)
assert.match(invoiceDetailPage, /executeAndroidPrint\([\s\S]*printCurrentPageDocument\('kubri-print-root', 'invoice', validate\)/)
assert.match(invoiceDetailPage, /executeAndroidPrint\([\s\S]*printCurrentPageDocument\('kubri-print-root', 'receipt', validate\)/)
assert.match(androidPrintExecution, /validateRenderedPrintOutput/)
assert.match(androidPrintExecution, /PRINT_EMPTY_RENDER/)
...
assert.match(branchDashboard, /\[t\('recent\.invoice'\), t\('recent\.customer'\), t\('recent\.date'\), t\('recent\.time'\), t\('recent\.amount'\), t\('recent\.status'\)\]/)
...
assert.match(invoiceSettings, /DocumentStudioSectionNav/)
...
assert.match(css, /--font-arabic: 'Cairo', 'KubriArabic'/)
assert.match(css, /html\[dir='rtl'\] body \{[\s\S]*font-synthesis: none/)
...
assert.match(workspace, /<DocumentStudioHeader/)
assert.match(invoiceSettings, /DocumentStudioWorkspace/)
assert.match(invoiceSettings, /DocumentStudioPreviewToolbar/)
assert.match(invoiceSettings, /DocumentStudioActionFooter/)
...
for (const marker of ['thermal-classic-line', 'thermal-compact-line', 'thermal-structured-line__figures', 'thermal-branded-card']) assert.match(thermalCompositions, new RegExp(marker))
...
assert.match(thermalCompositions, /thermal-classic-line__names--combined/)
assert.match(thermalCompositions, /const primary = item\.description\.trim\(\)/)
assert.match(thermalCompositions, /const secondary = item\.descriptionAr\?\.trim\(\) \?\? ''/)
assert.match(thermalCompositions, /!sameIdentity\(primary, secondary\)/)
assert.match(thermalCompositions, /thermal-classic-line__name-segment--en" dir="ltr"/)
assert.match(thermalCompositions, /thermal-classic-line__name-segment--ar" dir="rtl"/)
assert.match(thermalCompositions, /function ClassicVatLabel/)
assert.match(thermalCompositions, /function ClassicGrandTotalLabel/)
...
assert.match(css, /thermal-theme--classic \.thermal-classic-line__names--combined/)
assert.match(css, /thermal-theme--classic \.thermal-classic-line__name-segment\{font-weight:inherit;unicode-bidi:isolate/)
assert.match(css, /thermal-theme--classic \.thermal-classic-line__values\{display:grid!important/)
assert.match(css, /thermal-theme--classic \.thermal-classic-totals \.thermal-row-strong\{display:grid/)
assert.match(css, /thermal-theme--classic \.thermal-classic-total-label\{display:grid;justify-items:start/)
assert.match(css, /thermal-receipt--58mm\.thermal-theme--classic \.thermal-classic-total-label\{overflow-wrap:normal/)
...
assert.match(thermalCompositions, /function CompactMetadataRow/)
assert.match(thermalCompositions, /function CompactMetadataLabel/)
...
assert.match(css, /thermal-theme--compact-retail \.thermal-compact-meta__row\{display:grid;grid-template-columns:minmax\(0,1fr\) auto/)
assert.match(css, /thermal-theme--compact-retail \.thermal-compact-meta__value\{justify-self:end;white-space:nowrap;text-align:end/)
...
assert.match(css, /thermal-receipt--58mm\.thermal-theme--compact-retail \.thermal-compact-meta__row\{gap:1mm/)
...
assert.match(thermalCompositions, /thermal-structured-line__names--combined/)
assert.match(thermalCompositions, /thermal-structured-line__name-segment--en" dir="ltr"/)
assert.match(thermalCompositions, /thermal-structured-line__name-segment--ar" dir="rtl"/)
assert.match(thermalCompositions, /function StructuredReceiptMetadata/)
assert.match(thermalCompositions, /function StructuredGrandTotalLabel/)
...
assert.match(thermalCompositions, /thermal-structured-masthead--split/)
assert.match(thermalCompositions, /thermal-structured-identity/)
assert.match(thermalCompositions, /model\.presentation\.logo\.visible && Boolean\(receipt\.logoUrl\)/)
assert.doesNotMatch(structuredComposition, /thermal-structured-ledger/)
assert.match(css, /thermal-theme--structured-detail \.thermal-structured-masthead--split\{display:grid;grid-template-columns:minmax\(0,\.82fr\) minmax\(0,1\.18fr\)/)
assert.match(css, /thermal-theme--structured-detail \.thermal-structured-items-heading\{margin-block-start:1\.5mm;padding:\.8mm 0;border-block:2px solid/)
assert.match(css, /thermal-theme--structured-detail \.thermal-structured-line__names--combined\{display:block/)
assert.match(css, /thermal-theme--structured-detail \.thermal-structured-accounting\{display:grid;grid-template-columns/)
assert.match(css, /thermal-theme--structured-detail \.thermal-structured-totals \.thermal-row-strong\{display:grid/)
assert.match(css, /thermal-theme--structured-detail \.thermal-structured-close\{grid-template-columns:28mm minmax\(0,1fr\);margin-block-start:1\.5mm;padding:0;border:0/)
assert.match(css, /thermal-receipt--58mm\.thermal-theme--structured-detail \.thermal-structured-masthead--split,.thermal-receipt--58mm\.thermal-theme--structured-detail \.thermal-structured-close\{grid-template-columns:1fr/)
assert.match(css, /thermal-receipt--58mm\.thermal-theme--structured-detail \.thermal-structured-meta__row\{gap:1mm/)
assert.match(css, /thermal-theme--structured-detail \.thermal-legal-info bdi\[dir="ltr"\],\.thermal-theme--structured-detail \.thermal-buyer bdi\[dir="ltr"\]\{white-space:nowrap;unicode-bidi:isolate/)
assert.match(css, /thermal-theme--structured-detail \.thermal-structured-meta__row\{border-block-end-color:#aaa/)
assert.match(css, /thermal-theme--structured-detail \.thermal-structured-line__figures small\{color:#555;font-weight:400/)
assert.match(css, /thermal-theme--structured-detail \.thermal-structured-line__formula>strong\{font-weight:600/)
assert.match(css, /thermal-theme--structured-detail \.thermal-structured-line__amount>strong\{font-weight:800/)
assert.match(css, /thermal-theme--structured-detail \.thermal-structured-accounting\{grid-template-columns:minmax\(0,1\.28fr\) minmax\(0,\.72fr\)/)
assert.match(css, /thermal-theme--structured-detail \.thermal-structured-total-label\{gap:0;font-size:calc\(var\(--thermal-font\)\*\.92\);line-height:1\.13/)
assert.match(css, /thermal-theme--structured-detail \.thermal-structured-payments \.thermal-payment-kind\{padding-block-end:\.45mm;margin-block-end:\.35mm/)
assert.match(css, /thermal-theme--structured-detail \.thermal-structured-close\{margin-block-start:\.8mm/)
assert.match(css, /thermal-receipt--58mm\.thermal-theme--structured-detail \.thermal-structured-accounting\{grid-template-columns:1fr/)
const brandedComposition = thermalCompositions.slice(thermalCompositions.indexOf('export function BrandedModernReceipt'), thermalCompositions.indexOf('const THERMAL_LAYOUT_RENDERERS'))
let brandedMarker = -1
for (const marker of ['<BrandedSellerHeader', 'thermal-branded-document', '<ReceiptBuyer', '<BrandedItems', '<BrandedTotals', '<BrandedPayments', '<Verification', '<BrandedFooter']) {
  const nextMarker = brandedComposition.indexOf(marker)
  assert.ok(nextMarker > brandedMarker, `Branded must preserve its premium receipt order at ${marker}`)
  brandedMarker = nextMarker
}
const brandedHeader = thermalCompositions.slice(thermalCompositions.indexOf('function brandedMerchantNames'), thermalCompositions.indexOf('function BrandedItems'))
assert.match(brandedHeader, /seller\.displayHeading/)
assert.match(brandedHeader, /seller\.branch\.visible/)
assert.match(brandedHeader, /!sameIdentity\(candidate, seller\.registeredName\)/)
assert.match(brandedHeader, /<LegalSeller receipt=\{receipt\} contact website=\{false\} \/>/)
assert.match(thermalCompositions, /function BrandedReceiptMetadata/)
assert.match(thermalCompositions, /function BrandedGrandTotalLabel/)
assert.match(thermalCompositions, /thermal-branded-card__number/)
assert.match(thermalCompositions, /thermal-branded-card__names--combined/)
assert.match(thermalCompositions, /thermal-branded-card__name-segment--en" dir="ltr"/)
assert.match(thermalCompositions, /thermal-branded-card__name-segment--ar" dir="rtl"/)
assert.match(thermalCompositions, /thermal-branded-card__discount/)
assert.match(thermalCompositions, /thermal-branded-payments--\$\{positivePayments\.length > 1 \? 'split' : 'single'\}/)
assert.match(thermalCompositions, /function BrandedFooter/)
assert.match(css, /thermal-theme--branded-modern \.thermal-branded-masthead\{padding:1\.8mm;border:2px solid #222/)
assert.match(css, /thermal-theme--branded-modern \.thermal-branded-document\{display:grid;grid-template-columns:minmax\(0,\.84fr\) minmax\(0,1\.16fr\);margin-block:1\.8mm;border:2px solid #222/)
assert.match(css, /thermal-theme--branded-modern \.thermal-branded-card\{padding:1\.5mm;border:1px solid #666/)
assert.match(css, /thermal-theme--branded-modern \.thermal-branded-card__number\{display:grid;place-items:center/)
assert.match(css, /thermal-theme--branded-modern \.thermal-branded-card__names--combined\{display:block/)
assert.match(css, /thermal-theme--branded-modern \.thermal-branded-totals \.thermal-row-strong\{display:grid/)
assert.match(css, /thermal-theme--branded-modern \.thermal-branded-total-label\{display:grid;justify-items:start/)
assert.match(css, /thermal-theme--branded-modern \.thermal-branded-payments\{border-color:#888/)
assert.match(css, /thermal-theme--branded-modern \.thermal-branded-close\{gap:1mm;margin-block-start:1\.2mm;padding:1\.2mm 0 0;border:0;border-block-start:1px solid #888/)
assert.match(css, /thermal-theme--branded-modern \.thermal-legal-info bdi\[dir="ltr"\],\.thermal-theme--branded-modern \.thermal-buyer bdi\[dir="ltr"\]\{white-space:nowrap;unicode-bidi:isolate/)
assert.match(css, /thermal-receipt--58mm\.thermal-theme--branded-modern \.thermal-branded-document\{grid-template-columns:1fr/)
