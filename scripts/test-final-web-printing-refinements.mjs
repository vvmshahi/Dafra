...
assert.match(posPage, /isAndroidBrowser\(\)[\s\S]*openPrintPopup/)
assert.match(invoiceDetailPage, /printCurrentPageDocument\('kubri-print-root', 'invoice'\)/)
assert.match(invoiceDetailPage, /printCurrentPageDocument\('kubri-print-root', 'receipt'\)/)
...
assert.match(branchDashboard, /\[t\('recent\.invoice'\), t\('recent\.customer'\), t\('recent\.dateTime'\), t\('recent\.amount'\), t\('recent\.status'\)\]/)
assert.match(branchDashboard, /data-branch-title-block/)
assert.match(branchDashboard, /dir="ltr" className="relative mx-auto/)
assert.match(branchDashboard, /formatSaudiDateTime\(invoiceTimestamp/)
