...
for (const tab of ['overview', 'products', 'credit']) assert.match(detail, new RegExp(`'${tab}'`), `customer profile has ${tab} tab`)
...
for (const tab of ['overview', 'payments']) assert.match(workspace, new RegExp(`'${tab}'`), `Customer Credit has ${tab} workspace tab`)
assert.doesNotMatch(workspace, /id: 'customers'/)
assert.doesNotMatch(workspace, /id: 'statements'/)
