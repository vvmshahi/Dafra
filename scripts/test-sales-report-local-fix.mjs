...
assert.match(sales, /function SalesKpi/)
assert.match(sales, /lg:grid-cols-6/)
assert.match(sales, /function displayPercent/)
assert.match(sales, /Math\.abs\(value\) < 0\.5 \? '0%'/)
assert.match(sales, /lg:grid-cols-2 lg:items-stretch/)
assert.match(sales, /card flex flex-col overflow-hidden lg:h-\[29rem\]/)
assert.match(sales, /lg:min-h-0 lg:flex-1 lg:overflow-y-auto/)
assert.match(sales, /tabIndex=\{0\} aria-label=\{t\('sales\.topItems'\)\}/)
assert.match(sales, /const topCategory = data\.catPerformance\.reduce/)
assert.match(sales, /data\.catPerformance\.length/)
assert.match(sales, /topCategory\.pct\.toFixed\(1\)\}%/)
