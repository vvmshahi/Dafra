...
BEGIN
  ...
  RETURN jsonb_build_object('schema_version',1,'identity',jsonb_build_object('display_heading',b.display_name,'display_subheading',null,'custom_display_name',b.display_name,'show_company_name',true,'show_branch_name',true),'contact',jsonb_build_object('phone',b.phone,'email',b.email,'website',b.website,'address_override',null,'show_phone',b.phone is not null,'show_email',coalesce(b.show_email,false),'show_website',coalesce(b.show_website,false),'show_address',true),'footer',jsonb_build_object('thank_you_message',null,'footer_note',b.receipt_footer,'refund_note',null,'show_thank_you',false,'show_footer',coalesce(b.show_footer,true),'show_refund_note',false),'logo',jsonb_build_object('visible',coalesce(b.show_logo,false),'asset_path',b.logo_url,'asset_version',1,'size','medium'),'thermal',jsonb_build_object('width','80mm','density','standard','qr_size','standard','qr_alignment','center','wrap_item_names',true,'show_cash_change',coalesce(b.show_cash_change,true)),'a4',jsonb_build_object('template_id','classic','template_version',1,'header_style','standard'),'after_sale_action',CASE WHEN b.print_mode='pdf' THEN 'a4' ELSE 'receipt' END);
END $$;

CREATE OR REPLACE FUNCTION public.get_branch_invoice_settings(p_branch_id uuid)
RETURNS jsonb LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path=public SET row_security=off AS $$
DECLARE u record; b record; settings jsonb;
BEGIN
  ...
  settings := coalesce(b.presentation_settings, public.v1_default_invoice_presentation_settings(b.id));
  settings := jsonb_set(settings, '{contact,address_override}', coalesce(settings->'contact'->'address_override','null'::jsonb), true);
  settings := jsonb_set(settings, '{after_sale_action}', coalesce(settings->'after_sale_action', CASE WHEN b.print_mode='pdf' THEN '"a4"'::jsonb ELSE '"receipt"'::jsonb END), true);
  RETURN jsonb_build_object('branch_id',b.id,'presentation_settings',settings,'invoice_language',b.invoice_language,'print_mode',b.print_mode,'can_edit',true,'role',u.role);
END $$;

CREATE OR REPLACE FUNCTION public.update_branch_invoice_settings(p_payload jsonb)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path=public SET row_security=off AS $$
DECLARE u record; b record; lang text; mode text; incoming jsonb; settings jsonb; action text; override text;
BEGIN
  ...
  IF incoming IS NOT NULL THEN
    IF jsonb_typeof(incoming)<>'object' OR incoming ?| ARRAY['compliance','vatNumber','registrationIdentifier'] THEN RAISE EXCEPTION 'Invalid presentation settings' USING ERRCODE='22023'; END IF;
    action:=incoming->>'after_sale_action';
    IF action IS NOT NULL AND action NOT IN ('ask','receipt','a4','none') THEN RAISE EXCEPTION 'Invalid after-sale action' USING ERRCODE='22023'; END IF;
    IF incoming->'contact' ? 'address_override' THEN
      IF jsonb_typeof(incoming->'contact'->'address_override') NOT IN ('string','null') THEN RAISE EXCEPTION 'Invalid address override' USING ERRCODE='22023'; END IF;
      override:=nullif(left(btrim(incoming->'contact'->>'address_override'),500),'');
      incoming:=jsonb_set(incoming,'{contact,address_override}',to_jsonb(override),true);
    END IF;
