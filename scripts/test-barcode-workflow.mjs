...
assert.match(scanner, /this\.tail = this\.tail\.then\(task, task\)/)

assert.match(pos, /resolve_product_unit_barcode/)
assert.match(pos, /addQuantityToCart\(product, 1, unit\)/)
assert.match(pos, /now - previous\.at < 120/)
assert.match(pos, /receipt \|\| showOpenSession \|\| showCloseSession \|\| unitChooserProduct/)
assert.match(pos, /lookupDurationMs/)
...
assert.match(manager, /record_product_barcode_print/)
...
assert.match(labels, /width: \$\{width\}mm/)
assert.match(labels, /Array\.from\(\{ length: copies \}/)
