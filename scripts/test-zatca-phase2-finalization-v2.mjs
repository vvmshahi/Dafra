...
await test('27 frontend production reads safe status instead of raw v2 invoice columns', () => {
  ...
  assert.match(submission, /auth\.getSession\(\)/)
  assert.match(submission, /Authorization: `Bearer \$\{accessToken\}`/)
  assert.match(submission, /auth\.refreshSession\(\)/)
...
await test('thermal and A4 printing require the finalized rendered QR', () => {
  ...
  assert.match(invoiceDetail, /async function handlePrintA4\(\)[\s\S]*?openPrintPopup\(/)
  assert.match(invoiceDetail, /async function handlePrintThermal\(\)[\s\S]*?openBrowserReceiptPrint\(invoice\.id\)/)
