...
assert.match(detail, /<A4Document model=\{documentViewModel\} options=\{\{ pdfMode: true/)
...
assert.match(detail, /renderStoredQrDataUrl\(/)
...
assert.match(detail, /printReceiptInHiddenFrame\(invoice\.id\)/)
...
assert.match(thermal, /const isStandard = identity\.invoiceType === 'standard'/)
assert.doesNotMatch(thermal, /isAdjustment && buyer\.type/)
