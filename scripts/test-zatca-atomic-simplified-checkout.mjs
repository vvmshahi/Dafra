...
await test('SQL final transaction and constraints encode the atomic visibility invariant', () => {
  ...
  assert.doesNotMatch(migration, /0121e5c8|3ae21515|INV-0826|INV-0827/)
...
await test('preparation reuses authoritative commercial validation without visibility', () => {
  ...
  assert.match(migration, /public\.pos_checkout\(jsonb\)[\s\S]*b810798d8d9b64248f06ae67c6d95f90/)
  assert.match(migration, /public\.create_partial_credit_note\(jsonb\)[\s\S]*2789273cfedb900ae02d178eded85f90/)
...
await test('rollout fallback is write-free and committed replay wins even after flag rollback', () => {
  ...
  const rolloutCheck = edge.indexOf("from('zatca_atomic_checkout_branch_gates_v2')", handlerStart)
  ...
  assert.ok(rolloutCheck > 0 && rolloutCheck < prepareCall)
