...
test('customer type remains an accessible, non-colour-only radio choice', () => {
  ...
  assert.match(modal, /useState<CustomerType \| null>\(null\)/)
  assert.match(modal, /customer\?\.customer_type/)
  assert.match(modal, /Choose the customer type|customers:modal\.chooseType/)
})
...
test('individual and business sections are conditional and use the existing fields', () => {
  ...
  assert.doesNotMatch(modal, /customer-name-ar|customer-business-name-ar/)
...
test('POS refresh mapping and customer filter/count logic remain present', () => {
  ...
  assert.match(page, /actions=\{<Button size="sm" className="bg-\[#173f2a\]/)
  assert.doesNotMatch(page, /className="ms-auto" onClick=\{openAdd\}/)
})
...
test('English and Arabic modal, readiness, duplicate, empty and success copy exists', () => {
  ...
  assert.equal(en.modal.chooseType, 'Choose the customer type.')
