...
assert.match(detail, /nonFiscalDemo \?[\s\S]*DEMO — NOT A TAX INVOICE/)
assert.match(a4, /options\.nonFiscalDemo \? \['DEMO — NOT A TAX INVOICE', 'تجريبي — ليست فاتورة ضريبية'\]/)
assert.match(thermal, /ThermalReceiptCompositions/)
assert.match(thermalCompositions, /options\.nonFiscalDemo[\s\S]*DEMO — NOT A TAX INVOICE/)
