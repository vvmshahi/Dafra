...
const headerStart = dashboard.indexOf('data-branch-dashboard-header')
const headerSource = dashboard.slice(headerStart, dashboard.indexOf('</header>', headerStart))
assert.doesNotMatch(headerSource, /t\('branch\.newSale'\)/)
assert.doesNotMatch(headerSource, /navigate\('\/pos'\)/)
...
assert.match(dashboard, /border-\[#c8b16b\] bg-gradient-to-br/)
...
assert.match(dashboard, /session\?\.status === 'closed' \? t\('register\.closed'\) : t\('register\.opened'\)/)
