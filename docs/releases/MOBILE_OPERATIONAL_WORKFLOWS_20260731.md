...
### Server-contract implementation dependency

The reviewed implementation candidate is now isolated on `feature/purchase-posting-idempotency-20260731` as migration `20260731000100_purchase_posting_idempotency.sql`. It adds a durable operation table and immediate package-aware saleable-product receiving RPC while preserving the legacy web draft/confirmation path. It is **not** applied to the linked Supabase project: local Supabase/Docker execution is currently unresponsive, so database state tests, remote parity, and deployment are pending. Mobile purchase posting therefore remains gated; this branch must not call the candidate RPC until that server validation and deployment are complete.
