  ...
  return (
    <section className="mt-4 rounded-xl border border-gray-200 bg-white" aria-labelledby="statement-preview-title">
      ...
      <div className="grid gap-2 border-b border-gray-100 bg-gray-50/70 p-3 sm:grid-cols-4">
        <WorkspaceMetric label={t('metrics.balance')} value={<Rial amount={workspace.summary.balance} />} tone={workspace.summary.balance > 0 ? 'debt' : 'green'} />
...
export default function CustomerReceivablesReportPage() {
  ...
  return (
    <div className="mx-auto flex max-w-7xl flex-col gap-4">
      ...
      <FilterPanel id="customer-credit-filters" title={t('report.filters')} icon={Search} compact className="order-3 space-y-2">
        ...
        <div className="flex flex-wrap items-center justify-between gap-2"><span className="text-[11px] text-gray-500">{t('report.activityPeriod')}: {startDate} – {endDate} · {t('report.balanceAsOfToday')}</span><Button variant="secondary" size="sm" onClick={() => void refresh()} disabled={loading}>{loading ? <Loader2 size={14} className="animate-spin" /> : <RefreshCw size={14} />}{t('report.refresh')}</Button></div>
      </FilterPanel>
