...
const EXPENSE_CATEGORY_COLORS = ['#1B6B3A', '#4d8061', '#7f9f87', '#64748b', '#9aab9e']
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid gap-2 sm:grid-cols-3">
        ...
        <StatCard label={t('expenses:actualExpenses')} value={<Rial amount={data?.totalVariable ?? 0} />} accent="primary" sub={t('expenses.daily')} />
        <StatCard label={t('expenses:recurringEstimatedCosts')} value={<Rial amount={data?.totalFixed ?? 0} />} sub={t('expenses.recurring')} />
      </div>
      ...
      <div className="card p-4 space-y-3">
        ...
        {!data?.catBars.length ? <EmptyChart /> : (
          <ResponsiveContainer width="100%" height={Math.min(260, Math.max(180, data.catBars.length * 44 + 50))}>
            <BarChart data={data.catBars} layout="vertical"
              margin={{ top: 0, right: 20, bottom: 0, left: 100 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#f3f4f6" horizontal={false} />
              <XAxis type="number" tick={{ fontSize: 10, fill: '#9ca3af' }}
                tickFormatter={v => { const n = Number(v); return n >= 1000 ? `${(n / 1000).toLocaleString('en-US', { maximumFractionDigits: 1 })}K` : n.toLocaleString('en-US') }} />
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        <div className="card p-3">
          ...
          <p className="mt-1 text-lg font-bold text-[#1B6B3A]"><Rial amount={data?.totalVariable ?? 0} /></p>
          <p className="text-xs text-gray-400 mt-1">{data?.log.length ?? 0} expense entries</p>
          <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-gray-100">
            <div className="h-full rounded-full bg-[#1B6B3A] transition-all"
        ...
        <div className="card p-3">
          ...
          <p className="mt-1 text-lg font-bold text-slate-800"><Rial amount={data?.totalFixed ?? 0} /></p>
          <p className="text-xs text-gray-400 mt-1"><Rial amount={data?.monthlyFixed ?? 0} /> {t('expenses:perMonth')} {t('expenses:recurringMonthly')}</p>
          <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-gray-100">
            <div className="h-full rounded-full bg-slate-500 transition-all"
