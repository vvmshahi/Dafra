...
export default function BranchSettingsPage() {
  ...
  return <div className="mx-auto max-w-6xl space-y-5">
    ...
    <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      <nav className="flex gap-1 overflow-x-auto rounded-2xl border border-[#e8e1d1] bg-[#f4f0e7] p-1.5 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden" role="tablist" aria-label={t('workspace.title')}>
        {sections.map(item => { const Icon = item.icon; const selected = activeSection === item.id; return <button key={item.id} id={`branch-settings-tab-${item.id}`} type="button" role="tab" aria-selected={selected} aria-current={selected ? 'page' : undefined} aria-controls={`branch-settings-panel-${item.id}`} tabIndex={selected ? 0 : -1} onClick={() => selectSection(item.id)} className={`relative flex min-h-10 shrink-0 items-center gap-2 rounded-xl px-3 text-xs font-bold outline-none transition-[background-color,color,box-shadow,transform] duration-200 active:scale-[.98] focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#173f2a] sm:px-4 ${selected ? 'bg-[#173f2a] text-white shadow-[0_2px_8px_rgba(23,63,42,0.2)] after:absolute after:inset-x-3 after:bottom-1 after:h-0.5 after:rounded-full after:bg-[#e7ca78]' : 'text-[#526257] hover:bg-white/75 hover:text-[#173f2a]'}`}><Icon size={14} className={selected ? 'text-[#e7ca78]' : 'text-[#718078]'} /><span>{t(`workspace.sections.${item.id}`)}</span></button> })}
      </nav>
