...
export default function BranchSettingsPage() {
  ...
  const businessDisabled = creditSettings ? !creditSettings.tenantCreditEnabled : true
  ...
  async function saveCredit() {
    if (!selectedBranchId || !creditSettings || savingCredit || businessDisabled || !creditDirty) return
  ...
  return <div className="mx-auto max-w-6xl space-y-5">
    ...
    <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      <nav className="flex gap-1 overflow-x-auto border-b border-slate-100 bg-slate-50/70 p-2" role="tablist" aria-label={t('workspace.title')}>
        {sections.map(item => { const Icon = item.icon; const selected = activeSection === item.id; return <button key={item.id} type="button" role="tab" aria-selected={selected} aria-controls={`branch-settings-panel-${item.id}`} tabIndex={selected ? 0 : -1} onClick={() => selectSection(item.id)} className={`flex min-h-10 shrink-0 items-center gap-2 rounded-xl px-3.5 text-sm font-semibold outline-none transition-colors focus-visible:ring-2 focus-visible:ring-primary-500 ${selected ? 'bg-white text-primary-800 shadow-sm ring-1 ring-slate-200' : 'text-slate-500 hover:bg-white/80 hover:text-slate-800'}`}><Icon size={15} /><span>{t(`workspace.sections.${item.id}`)}</span></button> })}
      </nav>

      <main id={`branch-settings-panel-${activeSection}`} role="tabpanel" aria-labelledby={activeSection} className="p-4 sm:p-6">
