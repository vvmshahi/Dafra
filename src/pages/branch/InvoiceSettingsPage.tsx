...
import { immutableLogoObjectPath, resolveInvoicePresentationSettings, serializeInvoicePresentationSettingsForSave } from '@/lib/invoices/presentationSettings'
...
const ACCENT_PRESETS = ['#0f766e', '#172554', '#27272a', '#1d4ed8', '#047857', '#881337', '#6d28d9', '#c2410c', '#0e7490', '#000000']
...
function ThemeChoice({ label, value, options, onChange }: {
  ...
  return <fieldset><legend className="text-xs font-semibold text-gray-800">{label}</legend><div className="mt-2 grid gap-3 sm:grid-cols-2">{options.map(option => {
    ...
    return <button
      ...
      className={`group rounded-2xl border p-3 text-start outline-none transition-[border-color,background-color,box-shadow,transform] duration-150 ease-out active:scale-[.98] focus-visible:ring-2 focus-visible:ring-primary-500 ${selected ? 'border-primary-700 bg-primary-50 shadow-sm' : 'border-gray-200 bg-white hover:border-primary-300'}`}
    >
      <span className={`theme-thumb theme-thumb--${option.value}`} aria-hidden="true"><span /><span /><span /><span /></span>
      <span className="mt-3 flex items-center justify-between gap-3"><span className="text-xs font-bold text-gray-900">{option.label}</span>{selected && <Check size={14} className="text-primary-700" />}</span>
      <span className="mt-1 block text-[10px] leading-4 text-gray-500">{option.description}</span>
    </button>
  })}</div></fieldset>
}
...
function ReceiptThemeChoice({ value, onChange, t }: { value: ThermalDensity; onChange: (value: ThermalDensity) => void; t: (key: string) => string }) {
  ...
  return <fieldset><legend className="text-xs font-semibold text-gray-800">{t('printing:invoiceSettings.receiptThemes.title')}</legend><div className="mt-2 grid gap-2">{themes.map(theme => <button key={theme.id} type="button" role="radio" aria-checked={value === theme.id} onClick={() => onChange(theme.id)} className={`grid grid-cols-[64px_minmax(0,1fr)] gap-3 rounded-xl border p-3 text-start outline-none transition-[border-color,background-color,transform] duration-150 active:scale-[.98] focus-visible:ring-2 focus-visible:ring-primary-500 ${value === theme.id ? 'border-primary-700 bg-primary-50' : 'border-gray-200 hover:border-primary-300'}`}><span className={`receipt-theme-thumb receipt-theme-thumb--${theme.id}`} aria-hidden="true"><i /><i /><i /><i /></span><span><span className="flex items-center justify-between gap-2 text-xs font-bold text-gray-900">{theme.name}{value === theme.id && <Check size={13} className="text-primary-700" />}</span><span className="mt-1 block text-[10px] leading-4 text-gray-500">{theme.use}</span><span className="mt-1 block text-[9px] font-semibold uppercase tracking-wide text-primary-700">{theme.colour}</span></span></button>)}</div></fieldset>
}
...
export default function InvoiceSettingsPage({
  ...
  const [previewZoom, setPreviewZoom] = useState<A4PreviewZoom>('fit')
  ...
  async function uploadHeaderArtwork(file: File) {
    if (!draft || !branch || !profile?.tenant_id || !canEdit) return
    if (!LOGO_MIME_TYPES.includes(file.type) || file.size > LOGO_MAX_BYTES) { setSaveError(t('printing:invoiceSettings.a4.artworkInvalid')); return }
    setUploading(true); setSaveError(null)
    try {
      const extension = file.type === 'image/jpeg' ? 'jpg' : file.type === 'image/webp' ? 'webp' : 'png'
      const version = draft.presentation.a4.header_asset_version + 1
      const path = immutableLogoObjectPath(profile.tenant_id, branch.id, version, extension).replace(/logo\.(png|jpg|webp)$/, `header.${extension}`)
      const { error } = await supabase.storage.from('branch-assets').upload(path, file, { upsert: false }); if (error) throw error
      updateDraft(current => ({ ...current, presentation: { ...current.presentation, a4: { ...current.presentation.a4, header_asset_path: path, header_asset_version: version, header_asset_enabled: true } } }))
    } catch (error) { setSaveError(error instanceof Error ? error.message : t('printing:invoiceSettings.a4.artworkInvalid')) }
    finally { setUploading(false) }
  }
  ...
  const themeOptions: Array<{ value: A4TemplateId; label: string; description: string }> = [
    { value: 'classic', label: t('printing:invoiceSettings.a4.classic'), description: t('printing:invoiceSettings.a4.classicHelp') },
    { value: 'modern_split', label: t('printing:invoiceSettings.a4.modern'), description: t('printing:invoiceSettings.a4.modernHelp') },
    { value: 'minimal_professional', label: t('printing:invoiceSettings.a4.minimal'), description: t('printing:invoiceSettings.a4.minimalHelp') },
    { value: 'executive_green', label: t('printing:invoiceSettings.a4.executive'), description: t('printing:invoiceSettings.a4.executiveHelp') },
    { value: 'clean_ledger', label: t('printing:invoiceSettings.a4.ledger'), description: t('printing:invoiceSettings.a4.ledgerHelp') },
    { value: 'contemporary_border', label: t('printing:invoiceSettings.a4.contemporary'), description: t('printing:invoiceSettings.a4.contemporaryHelp') },
  ]
  ...
  return <div className={`mx-auto max-w-[1440px] space-y-5 ${embedded ? 'pb-24' : 'pb-28'}`}>
    ...
    <div className="grid items-start gap-5 xl:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
      <main className={`${mobilePane === 'preview' ? 'hidden xl:block' : ''} rounded-2xl border border-gray-200 bg-white p-4 shadow-sm sm:p-6`} role="tabpanel">
        ...
        {activeTab === 'a4' && <div className="space-y-6"><div><h2 className="text-base font-bold text-gray-950">{t('printing:invoiceSettings.tabs.a4')}</h2><p className="mt-1 text-xs leading-5 text-gray-500">{t('printing:invoiceSettings.a4.help')}</p></div><ThemeChoice label={t('printing:invoiceSettings.a4.layout')} value={p.a4.template_id} onChange={value => { updateSection('a4', 'template_id', value); setPreviewMode('a4') }} options={themeOptions} /><fieldset><legend className="text-xs font-semibold text-gray-800">{t('printing:invoiceSettings.a4.accent')}</legend><div className="mt-2 flex flex-wrap items-center gap-2">{ACCENT_PRESETS.map((colour, index) => <button key={`${colour}-${index}`} type="button" aria-label={`${t('printing:invoiceSettings.a4.accent')} ${index + 1}`} aria-pressed={p.a4.accent_color === colour} onClick={() => updateSection('a4', 'accent_color', colour)} className={`h-9 w-9 rounded-full border-4 outline-none focus-visible:ring-2 focus-visible:ring-primary-500 ${p.a4.accent_color === colour ? 'border-blue-600' : 'border-gray-200'}`} style={{ backgroundColor: colour }} />)}<input aria-label={t('printing:invoiceSettings.a4.customColour')} type="color" value={p.a4.accent_color} onChange={event => updateSection('a4', 'accent_color', event.target.value)} className="h-10 w-12 rounded border border-gray-200 bg-white p-1" /><input aria-label={t('printing:invoiceSettings.a4.hex')} value={p.a4.accent_color} onChange={event => { if (/^#[0-9a-f]{6}$/i.test(event.target.value)) updateSection('a4', 'accent_color', event.target.value.toLowerCase()) }} className="h-10 w-28 rounded-xl border border-gray-200 px-3 font-mono text-xs uppercase" /><button type="button" onClick={() => updateSection('a4', 'accent_color', '#0f766e')} className="text-xs font-semibold text-gray-600 underline">{t('printing:invoiceSettings.a4.resetColour')}</button></div></fieldset><section className="rounded-2xl border border-gray-200 bg-gray-50 p-4"><div className="flex items-center justify-between gap-3"><div><h3 className="text-xs font-bold text-gray-900">{t('printing:invoiceSettings.a4.artwork')}</h3><p className="mt-1 text-[11px] text-gray-500">{t('printing:invoiceSettings.a4.artworkHelp')}</p></div><button type="button" onClick={() => headerInput.current?.click()} disabled={uploading} className="inline-flex min-h-10 items-center gap-2 rounded-xl border border-gray-200 bg-white px-3 text-xs font-semibold"><Upload size={14} />{p.a4.header_asset_path ? t('printing:invoiceSettings.a4.replaceArtwork') : t('printing:invoiceSettings.a4.uploadArtwork')}</button></div><input ref={headerInput} type="file" className="sr-only" accept={LOGO_MIME_TYPES.join(',')} onChange={event => { const file = event.target.files?.[0]; if (file) void uploadHeaderArtwork(file); event.target.value = '' }} />{p.a4.header_asset_path && <div className="mt-4 space-y-3"><ToggleRow label={t('printing:invoiceSettings.a4.enableArtwork')} checked={p.a4.header_asset_enabled} onChange={value => updateSection('a4', 'header_asset_enabled', value)} /><Choice label={t('printing:invoiceSettings.a4.fit')} value={p.a4.header_asset_fit} onChange={value => updateSection('a4', 'header_asset_fit', value)} options={[{ value: 'contain', label: t('printing:invoiceSettings.a4.contain') }, { value: 'cover', label: t('printing:invoiceSettings.a4.cover') }]} /><label className="block text-xs font-semibold text-gray-800">{t('printing:invoiceSettings.a4.height')}<input type="range" min="18" max="56" value={p.a4.header_asset_height} onChange={event => updateSection('a4', 'header_asset_height', Number(event.target.value))} className="mt-2 w-full accent-primary-700" /></label><label className="block text-xs font-semibold text-gray-800">{t('printing:invoiceSettings.a4.spacing')}<input type="range" min="0" max="16" value={p.a4.header_asset_spacing} onChange={event => updateSection('a4', 'header_asset_spacing', Number(event.target.value))} className="mt-2 w-full accent-primary-700" /></label><button type="button" onClick={() => updateDraft(current => ({ ...current, presentation: { ...current.presentation, a4: { ...current.presentation.a4, header_asset_path: null, header_asset_enabled: false } } }))} className="inline-flex items-center gap-2 text-xs font-semibold text-red-700"><Trash2 size={14} />{t('printing:invoiceSettings.a4.removeArtwork')}</button></div>}</section></div>}
      ...
      <aside className={`${mobilePane === 'settings' ? 'hidden lg:block' : ''} lg:sticky lg:top-5`} aria-label={t('printing:invoiceSettings.livePreview')}><div className="rounded-2xl border border-gray-200 bg-[#f4f6f4] p-4"><div className="mb-3 flex flex-wrap items-center justify-between gap-3"><div className="flex items-center gap-2"><Eye size={16} className="text-primary-700" /><div><h2 className="text-sm font-bold text-gray-900">{t('printing:invoiceSettings.livePreview')}</h2><p className="text-[10px] text-gray-500">{t('printing:invoiceSettings.pageCount', { count: 1 })}</p></div></div>{previewMode === 'a4' && <div className="flex items-center rounded-lg bg-white p-1 ring-1 ring-gray-200"><button type="button" aria-label={t('printing:invoiceSettings.zoomOut')} onClick={() => setPreviewZoom(previewZoom === 1.25 ? 1 : previewZoom === 1 ? .75 : 'fit')} className="grid h-8 w-8 place-items-center rounded hover:bg-gray-50"><Minus size={14} /></button><button type="button" onClick={() => setPreviewZoom('fit')} className="h-8 px-2 text-[10px] font-bold text-gray-600">{previewZoom === 'fit' ? t('printing:invoiceSettings.fitPage') : `${previewZoom * 100}%`}</button><button type="button" aria-label={t('printing:invoiceSettings.zoomIn')} onClick={() => setPreviewZoom(previewZoom === 'fit' ? .75 : previewZoom === .75 ? 1 : 1.25)} className="grid h-8 w-8 place-items-center rounded hover:bg-gray-50"><Plus size={14} /></button></div>}</div>{previewMode === 'thermal' ? <div className="mx-auto max-w-full overflow-x-auto rounded-xl bg-gray-100 p-3"><ThermalReceipt model={previewModel} options={{ preview: true, qrImageUrl: null, sampleLabel: t('printing:preview') }} /></div> : <A4PreviewFit zoom={previewZoom} bounded><A4Document model={previewModel} options={{ preview: true, qrImageUrl: null, sampleLabel: t('printing:preview'), pageNumbers: true }} /></A4PreviewFit>}</div></aside>
    </div>
