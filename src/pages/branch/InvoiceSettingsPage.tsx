...
function fromResponse(value: unknown, branch: Branch): InvoicePresentationDraft {
  return normalizeDraft(normalizeInvoiceSettings(value, branch))
}
...
export default function InvoiceSettingsPage() {
  ...
  async function saveChanges() {
    ...
    } catch (error) { setSaveError(error instanceof Error ? error.message : t('settings:invoiceSettings.saveFailed')) }
  ...
  const logoSrc = p?.logo.asset_path ? supabase.storage.from('branch-assets').getPublicUrl(p.logo.asset_path).data.publicUrl : null
  ...
  const setUseBranchName = (enabled: boolean) => updateDraft(current => ({ ...current, presentation: { ...current.presentation, identity: { ...current.presentation.identity, display_heading: enabled ? branchName : null, custom_display_name: enabled ? null : current.presentation.identity.custom_display_name, show_branch_name: true } } }))
  ...
  const formatOptions = [{ value: 'thermal' as const, label: 'Thermal' }, { value: 'pdf' as const, label: 'A4' }]
  const actionOptions = [{ value: 'thermal' as const, label: 'Print receipt', description: 'Use the thermal receipt path' }, { value: 'pdf' as const, label: 'Print A4 invoice', description: 'Use the A4 invoice path' }, { value: 'both' as const, label: 'Print according to format', description: 'Keep both document formats available' }]
  ...
  return <div className="mx-auto max-w-[1440px] space-y-5 pb-28">
    ...
    <div className="grid items-start gap-6 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
      <main className={`${mobilePane === 'preview' ? 'hidden lg:block' : ''} rounded-2xl border border-gray-200 bg-white p-4 shadow-sm sm:p-6`} role="tabpanel">
        ...
        {activeTab === 'general' && <div className="space-y-5"><div><h2 className="text-base font-bold text-gray-950">General</h2><p className="mt-1 text-xs leading-5 text-gray-500">Choose the language and document path your staff use most.</p></div><Choice label="Invoice language" value={draft.invoiceLanguage} onChange={value => updateDraft(current => ({ ...current, invoiceLanguage: value }))} options={[{ value: 'en', label: 'English' }, { value: 'ar', label: 'Arabic' }, { value: 'both', label: 'Bilingual' }]} /><Choice label="Default document format" value={draft.printMode === 'pdf' ? 'pdf' : 'thermal'} onChange={value => updateDraft(current => ({ ...current, printMode: value }))} options={formatOptions} /><Choice label="Default action after sale" value={draft.afterSaleAction ?? 'receipt'} onChange={value => updateDraft(current => ({ ...current, afterSaleAction: value }))} options={actionOptions} /></div>}
        {activeTab === 'branding' && <div className="space-y-5"><div><h2 className="text-base font-bold text-gray-950">Header & Branding</h2><p className="mt-1 text-xs leading-5 text-gray-500">Make the first impression clear and recognisable.</p></div><ToggleRow label="Use branch name" checked={useBranchName} onChange={setUseBranchName} help={branchName ? `Resolved branch: ${branchName}` : undefined} /><TextField id="display-heading" label="Custom display heading" value={p.identity.display_heading} disabled={useBranchName} onChange={value => updateSection('identity', 'display_heading', value)} placeholder="Shown at the top of the invoice" /><TextField id="display-subheading" label="Display subheading (optional)" value={p.identity.display_subheading} onChange={value => updateSection('identity', 'display_subheading', value)} placeholder="Leave blank to hide it" /><ToggleRow label="Show company name" checked={p.identity.show_company_name} onChange={value => updateSection('identity', 'show_company_name', value)} /><div className="rounded-2xl border border-dashed border-gray-300 p-4"><div className="flex items-center gap-4"><div className="grid h-20 w-24 shrink-0 place-items-center overflow-hidden rounded-xl bg-gray-50">{logoSrc ? <img src={logoSrc} alt="Current invoice logo" className="h-full w-full object-contain" /> : <FileText className="text-gray-300" />}</div><div className="min-w-0 flex-1"><p className="text-xs font-semibold text-gray-800">Logo</p><p className="mt-1 text-[11px] text-gray-500">Upload, replace, or remove the saved branch logo.</p><div className="mt-3 flex flex-wrap gap-2"><button type="button" disabled={uploading} onClick={() => logoInput.current?.click()} className="inline-flex min-h-10 items-center gap-2 rounded-xl border border-gray-200 px-3 py-2 text-xs font-semibold text-gray-700 hover:bg-gray-50 disabled:opacity-50"><Upload size={14} />{uploading ? 'Uploading…' : 'Replace logo'}</button><button type="button" disabled={!p.logo.asset_path} onClick={() => setConfirmRemoveLogo(true)} className="inline-flex min-h-10 items-center gap-2 rounded-xl border border-red-100 px-3 py-2 text-xs font-semibold text-red-700 hover:bg-red-50 disabled:opacity-50"><Trash2 size={14} />Remove</button></div><input ref={logoInput} type="file" className="sr-only" accept={LOGO_MIME_TYPES.join(',')} onChange={event => { const file = event.target.files?.[0]; if (file) void uploadLogo(file); event.target.value = '' }} /></div></div></div><Choice label="Logo size" value={p.logo.size} onChange={value => updateSection('logo', 'size', value)} options={[{ value: 'small', label: 'Small' }, { value: 'medium', label: 'Medium' }, { value: 'large', label: 'Large' }]} /></div>}
        {activeTab === 'contact' && <div className="space-y-5"><div><h2 className="text-base font-bold text-gray-950">Contact & Footer</h2><p className="mt-1 text-xs leading-5 text-gray-500">Only enabled, non-empty contact details appear in the document. Leave address blank to inherit from the branch profile.</p></div><TextField id="phone" label="Phone override" value={p.contact.phone} onChange={value => updateSection('contact', 'phone', value)} placeholder={branch?.phone ?? 'Inherit branch phone'} /><ToggleRow label="Show phone" checked={p.contact.show_phone} disabled={!p.contact.phone} onChange={value => updateSection('contact', 'show_phone', value)} /><TextField id="email" label="Email" value={p.contact.email} onChange={value => updateSection('contact', 'email', value)} placeholder="hello@example.com" /><ToggleRow label="Show email" checked={p.contact.show_email} disabled={!p.contact.email} onChange={value => updateSection('contact', 'show_email', value)} /><TextField id="website" label="Website" value={p.contact.website} onChange={value => updateSection('contact', 'website', value)} placeholder="https://example.com" /><ToggleRow label="Show website" checked={p.contact.show_website} disabled={!p.contact.website} onChange={value => updateSection('contact', 'show_website', value)} /><TextField id="address-override" label="Address override" value={p.contact.address_override} onChange={value => updateSection('contact', 'address_override', value)} placeholder={branch?.address ?? 'Inherit branch address'} multiline /><ToggleRow label="Show address" checked={p.contact.show_address} onChange={value => updateSection('contact', 'show_address', value)} /><TextField id="footer-message" label="Footer message" value={p.footer.footer_note} onChange={value => updateSection('footer', 'footer_note', value)} placeholder="Leave blank to hide the footer message" multiline /></div>}
