      ...
      <aside className={`${mobilePane === 'settings' ? 'hidden lg:block' : ''} xl:sticky xl:top-0`} aria-label={t('printing:invoiceSettings.livePreview')}>
        <div className="rounded-2xl border border-gray-200 bg-[#f1f5f2] p-3">
          <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-center gap-2"><Eye size={16} className="text-primary-700" /><div><h2 className="text-sm font-bold text-gray-900">{t('printing:invoiceSettings.livePreview')}</h2><p className="text-[10px] text-gray-500">{t('printing:invoiceSettings.pageCount', { count: 1 })}</p></div></div>
          ...
          {previewMode === 'thermal' ? <div className="mx-auto max-w-full overflow-x-auto rounded-xl bg-gray-100 p-3"><ThermalReceipt model={previewModel} options={{ preview: true, qrImageUrl: null, sampleLabel: t('printing:preview') }} /></div> : <A4PreviewFit zoom={previewZoom} bounded><A4Document model={previewModel} options={{ preview: true, qrImageUrl: previewQrUrl, sampleLabel: t('printing:preview'), pageNumbers: true, nonFiscalDemo: previewQrState === 'demo' }} /></A4PreviewFit>}
        </div>
      </aside>
    </div>
