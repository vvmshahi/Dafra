...
import type { A4TemplateId, Branch, InvoicePresentationSettings } from '@/types/database'
...
function TabButton({ active, label, onClick }: { active: boolean; label: string; onClick: () => void }) {
  return <button type="button" role="tab" aria-selected={active} onClick={onClick} className={`min-h-10 whitespace-nowrap rounded-xl px-4 py-2 text-xs font-bold outline-none transition-[background-color,color,transform,box-shadow] duration-150 ease-out active:scale-[.97] focus-visible:ring-2 focus-visible:ring-gold-300 focus-visible:ring-offset-2 focus-visible:ring-offset-sidebar ${active ? 'bg-white text-sidebar shadow-sm' : 'text-white/80 hover:bg-white/10 hover:text-white'}`}>{label}</button>
}
...
export default function InvoiceSettingsPage({
  ...
  return <div className={`mx-auto max-w-[1440px] space-y-5 ${embedded ? 'pb-24' : 'pb-28'}`}>
    ...
    <div className="overflow-x-auto rounded-2xl border border-sidebar bg-sidebar p-1.5 shadow-sm" role="tablist" aria-label={t('printing:invoiceSettings.sections')}><div className="flex min-w-max gap-1.5">{tabs.map(tab => <TabButton key={tab.id} active={activeTab === tab.id} label={tab.label} onClick={() => { setActiveTab(tab.id); setMobilePane('settings') }} />)}</div></div>
    ...
    <div className="grid items-start gap-5 xl:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
      <main className={`${mobilePane === 'preview' ? 'hidden xl:block' : ''} rounded-2xl border border-gray-200 bg-white p-4 shadow-sm sm:p-6`} role="tabpanel">
        ...
        {activeTab === 'thermal' && <div className="space-y-5"><div><h2 className="text-base font-bold text-gray-950">{t('printing:invoiceSettings.tabs.thermal')}</h2><p className="mt-1 text-xs leading-5 text-gray-500">{t('printing:invoiceSettings.thermal.help')}</p></div><Choice label={t('printing:invoiceSettings.thermal.paperWidth')} value={p.thermal.width} onChange={value => updateSection('thermal', 'width', value)} options={[{ value: '58mm', label: '58 mm' }, { value: '80mm', label: '80 mm' }]} /><Choice label={t('printing:invoiceSettings.thermal.density')} value={p.thermal.density} onChange={value => updateSection('thermal', 'density', value)} options={[{ value: 'compact', label: t('printing:invoiceSettings.values.compact') }, { value: 'standard', label: t('printing:invoiceSettings.values.standard') }, { value: 'detailed', label: t('printing:invoiceSettings.values.detailed') }]} /><Choice label={t('printing:invoiceSettings.thermal.qrSize')} value={p.thermal.qr_size} onChange={value => updateSection('thermal', 'qr_size', value)} options={[{ value: 'small', label: t('printing:invoiceSettings.values.small') }, { value: 'standard', label: t('printing:invoiceSettings.values.medium') }, { value: 'large', label: t('printing:invoiceSettings.values.large') }]} /><ToggleRow label={t('printing:invoiceSettings.thermal.wrapNames')} checked={p.thermal.wrap_item_names} onChange={value => updateSection('thermal', 'wrap_item_names', value)} /><ToggleRow label={t('printing:invoiceSettings.thermal.showChange')} checked={p.thermal.show_cash_change} onChange={value => updateSection('thermal', 'show_cash_change', value)} /></div>}
        {activeTab === 'a4' && <div className="space-y-5"><div><h2 className="text-base font-bold text-gray-950">{t('printing:invoiceSettings.tabs.a4')}</h2><p className="mt-1 text-xs leading-5 text-gray-500">{t('printing:invoiceSettings.a4.help')}</p></div><ThemeChoice label={t('printing:invoiceSettings.a4.theme')} value={p.a4.template_id} onChange={value => { updateSection('a4', 'template_id', value); setPreviewMode('a4') }} options={themeOptions} /></div>}
      ...
      <aside className={`${mobilePane === 'settings' ? 'hidden lg:block' : ''} lg:sticky lg:top-5`} aria-label={t('printing:invoiceSettings.livePreview')}><div className="rounded-2xl border border-gray-200 bg-[#f4f6f4] p-4 sm:p-6"><div className="mb-4 flex flex-wrap items-center justify-between gap-3"><div className="flex items-center gap-2"><Eye size={16} className="text-primary-700" /><div><h2 className="text-sm font-bold text-gray-900">{t('printing:invoiceSettings.livePreview')}</h2><p className="text-[10px] text-gray-500">{t('printing:invoiceSettings.previewHelp')}</p></div></div><div className="flex rounded-xl bg-white p-1 ring-1 ring-gray-200"><button type="button" onClick={() => setPreviewMode('thermal')} className={`rounded-lg px-3 py-1.5 text-[11px] font-semibold ${previewMode === 'thermal' ? 'bg-primary-700 text-white' : 'text-gray-500'}`}>{t('printing:invoiceSettings.previewThermal')}</button><button type="button" onClick={() => setPreviewMode('a4')} className={`rounded-lg px-3 py-1.5 text-[11px] font-semibold ${previewMode === 'a4' ? 'bg-primary-700 text-white' : 'text-gray-500'}`}>{t('printing:invoiceSettings.previewA4')}</button></div></div>{previewMode === 'thermal' ? <div className="mx-auto max-w-full overflow-x-auto rounded-xl bg-gray-100 p-3"><ThermalReceipt model={previewModel} options={{ preview: true, qrImageUrl: null, sampleLabel: t('printing:preview') }} /></div> : <A4PreviewFit><A4Document model={previewModel} options={{ preview: true, qrImageUrl: null, sampleLabel: t('printing:preview'), pageNumbers: true }} /></A4PreviewFit>}</div></aside>
    </div>
