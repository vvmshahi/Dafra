...
async function hasPrintableHeaderDimensions(file: File) {
  const url = URL.createObjectURL(file)
  try {
    const image = new Image()
    image.src = url
    await image.decode()
    return image.naturalWidth >= 1200 && image.naturalHeight >= 120 && image.naturalWidth / image.naturalHeight >= 2
  } catch {
    return false
  } finally {
...
function ThemeChoice({ label, value, options, onChange }: {
  ...
  return <fieldset><legend className="text-xs font-semibold text-gray-800">{label}</legend><div className="mt-2 grid gap-3 sm:grid-cols-2">{options.map(option => {
    ...
    >
      <span className={`theme-thumb theme-thumb--${option.value}`} aria-hidden="true"><i className="thumb-brand" /><i className="thumb-title" /><i className="thumb-from" /><i className="thumb-to" /><i className="thumb-meta" /><i className="thumb-table" /><i className="thumb-qr" /><i className="thumb-total" /></span>
      <span className="mt-2 flex items-center justify-between gap-3"><span className="text-xs font-bold text-gray-900">{option.label}</span>{selected && <span className="grid h-5 w-5 place-items-center rounded-full bg-primary-700 text-white"><Check size={12} /></span>}</span>
...
export default function InvoiceSettingsPage({
  ...
  async function uploadHeaderArtwork(file: File) {
    ...
    if (!await hasPrintableHeaderDimensions(file)) { setSaveError(t('printing:invoiceSettings.a4.artworkInvalid')); return }
    setUploading(true); setSaveError(null)
    try {
      const extension = file.type === 'image/jpeg' ? 'jpg' : file.type === 'image/webp' ? 'webp' : 'png'
