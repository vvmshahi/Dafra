...
function ClassicItemName({ model, item }: { model: DocumentViewModel; item: DocumentViewModel['items'][number] }) {
  ...
  if (model.identity.language === 'both' && english && arabic && !identical) {
  ...
  const value = names(model, english, arabic)[0] ?? ''
  return <bdi className="a4-classic-item-name" dir={arabic && (!english || model.identity.language === 'ar') ? 'rtl' : 'ltr'}>{value}</bdi>
}
