    ...
    <div className="space-y-2">
      <div className="flex items-center gap-3">
        <div className="flex h-12 w-12 items-center justify-center rounded-xl border border-gray-200 bg-gray-50 text-2xl"
          aria-live="polite" aria-label={value ? `Selected category icon ${value}` : 'No category icon selected'}>
          {value || <span className="text-sm text-gray-300">None</span>}
        </div>
        <button ref={triggerRef} type="button" onClick={() => setOpen(true)}
          aria-haspopup="dialog" aria-expanded={open} aria-label="Choose category emoji"
          ...
          <SmilePlus size={16} /> Choose emoji
        </button>
        {value && (
          <button type="button" onClick={() => onChange('')}
            className="text-xs font-medium text-gray-400 hover:text-red-500" aria-label="Remove category icon">
            No icon
          </button>
        )}
      </div>
      ...
      {suggestions.length > 0 && (
        <div>
          <p className="mb-1 text-[11px] text-gray-400">Suggested from category name</p>
          <div className="flex gap-1.5">
            {suggestions.map(emoji => (
              <button key={emoji} type="button" onClick={() => select(emoji)}
                aria-label={`Use suggested emoji ${emoji}`}
      ...
      {open && (
        <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/35 p-3" role="presentation"
          ...
          <section role="dialog" aria-modal="true" aria-label="Choose category emoji"
            className="w-full max-w-[390px] overflow-hidden rounded-2xl border border-gray-200 bg-white p-3 shadow-2xl">
            <div className="mb-2 flex items-center justify-between px-1">
              <div>
                <h3 className="text-sm font-semibold text-gray-900">Choose category emoji</h3>
                <p className="text-[11px] text-gray-400">Search by English emoji name</p>
              </div>
              <button type="button" onClick={close} aria-label="Close emoji picker"
            ...
            {recent.length > 0 && (
              <div className="mb-2 rounded-xl bg-gray-50 p-2">
                <p className="mb-1.5 text-[10px] font-semibold uppercase tracking-wide text-gray-400">Recently used</p>
                <div className="flex flex-wrap gap-1">
                  {recent.map(emoji => (
                    <button key={emoji} type="button" onClick={() => select(emoji)}
                      aria-label={`Use recently selected emoji ${emoji}`}
            ...
            <Suspense fallback={
              <div className="flex h-[420px] items-center justify-center text-sm text-gray-400">
                <Search size={16} className="mr-2" /> Loading emoji search…
              </div>
            }>
              <EmojiPicker
