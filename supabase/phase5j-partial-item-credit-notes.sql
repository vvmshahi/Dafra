-- ============================================================
-- Phase 5J: partial item-level credit notes
-- Apply manually after reviewing on staging.
-- ============================================================
--
-- This migration adds backend-controlled partial credit-note creation.
-- It does not apply automatically, does not call ZATCA, and does not
-- modify existing original invoices.
--
-- Flow:
--   public.get_invoice_refundable_items(p_invoice_id uuid)
--   public.create_partial_credit_note(p_payload jsonb)
--
-- The create RPC independently revalidates item quantities and creates:
--   - a credit-note invoice row
--   - selected credit-note invoice_items only
--   - payment_refunds for the partial total
--   - stock movements for returned tracked stock only
...
DROP INDEX IF EXISTS public.invoices_original_full_credit_note_once_idx;

CREATE INDEX IF NOT EXISTS invoices_original_credit_notes_active_idx
...
CREATE OR REPLACE FUNCTION public.get_invoice_refundable_items(p_invoice_id UUID)
RETURNS TABLE (
...
REVOKE ALL ON FUNCTION public.get_invoice_refundable_items(UUID) FROM PUBLIC;
...
ALTER FUNCTION public.get_invoice_refundable_items(UUID) SET row_security = off;
...
COMMENT ON FUNCTION public.get_invoice_refundable_items(UUID) IS
  'Returns per-line remaining refundable quantity and amounts for an original invoice, scoped to the authenticated tenant/branch.';

COMMENT ON FUNCTION public.create_partial_credit_note(JSONB) IS
  'Creates a partial item-level credit note for selected original invoice items. Backend-controlled, idempotent, tenant/branch scoped, and concurrency-safe.';

COMMENT ON FUNCTION public.create_full_credit_note(JSONB) IS
  'Compatibility wrapper that creates a credit note for all remaining refundable quantities using create_partial_credit_note.';
...
-- ============================================================
--
-- 1) Confirm functions exist and are executable:
--
-- SELECT
--   to_regprocedure('public.get_invoice_refundable_items(uuid)') IS NOT NULL AS helper_exists,
--   to_regprocedure('public.create_partial_credit_note(jsonb)') IS NOT NULL AS partial_rpc_exists,
--   has_function_privilege('authenticated', 'public.create_partial_credit_note(jsonb)', 'EXECUTE') AS authenticated_can_create_partial;
--
-- 2) Confirm the full-credit-only unique index is gone:
--
-- SELECT to_regclass('public.invoices_original_full_credit_note_once_idx') AS old_unique_index;
--
-- 3) Confirm replacement lookup indexes:
--
-- SELECT
--   to_regclass('public.invoices_original_credit_notes_active_idx') AS credit_notes_idx,
--   to_regclass('public.invoice_items_original_invoice_item_idx') AS item_link_idx;
