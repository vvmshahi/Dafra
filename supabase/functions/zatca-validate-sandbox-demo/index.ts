...
async function authorizeCaller(db: any, req: Request): Promise<{ serviceRole: boolean }> {
  const jwt = bearerToken(req)
  if (jwtRole(jwt) === 'service_role') return { serviceRole: true }
  ...
  const assignedBranchUser = profile.role === 'branch' && profile.branch_id === TRADING_BRANCH_ID
...
async function loadScope(db: any, invoiceId: string): Promise<{
  invoice: any
  branch: any
  credential: any
}> {
  const [tenantResult, branchResult, invoiceResult, credentialResult] = await Promise.all([
    ...
    `).eq('id', TRADING_BRANCH_ID).eq('tenant_id', DEMO_TENANT_ID)
    ...
    `).eq('id', invoiceId).eq('tenant_id', DEMO_TENANT_ID).eq('branch_id', TRADING_BRANCH_ID).maybeSingle(),
    ...
    `).eq('tenant_id', DEMO_TENANT_ID).eq('branch_id', TRADING_BRANCH_ID)
...
async function listBrowserResults(db: any, invoiceIds: string[]): Promise<Record<string, unknown>> {
  if (invoiceIds.length === 0) return { ok: true, attempts: {} }
  const { data: invoices, error: invoiceError } = await db.from('invoices').select('id')
    .eq('tenant_id', DEMO_TENANT_ID).eq('branch_id', TRADING_BRANCH_ID).in('id', invoiceIds)
...
async function authorizeInvoiceScope(db: any, invoiceId: string): Promise<{ eligible: boolean }> {
  const { data, error } = await db.from('invoices')
    .select('id,status,zatca_invoice_type,zatca_type_code,zatca_status,original_invoice_id')
    .eq('id', invoiceId).eq('tenant_id', DEMO_TENANT_ID).eq('branch_id', TRADING_BRANCH_ID).maybeSingle()
  ...
  if (data.zatca_invoice_type === 'simplified') {
    return { eligible: baseEligible && data.zatca_type_code === '388' }
  }
  ...
  const { data: originalAttempt, error: attemptError } = await db.from('zatca_sandbox_validation_attempts')
    ...
    .in('status', ['sandbox_validated', 'sandbox_validated_with_warnings']).maybeSingle()
  return { eligible: baseEligible && !attemptError && !!originalAttempt?.id }
}

async function connectionStatus(db: any): Promise<Record<string, unknown>> {
  ...
  return {
    ...
    connection: active ? 'Active for compliance validation' : 'Not active',
...
async function validateInvoice(db: any, invoiceId: string): Promise<Record<string, unknown>> {
  const { invoice: invoiceRow, branch, credential } = await loadScope(db, invoiceId)
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!body || typeof body !== 'object' || Array.isArray(body) || Object.keys(body).some(key => !['action', 'invoiceId', 'invoiceIds'].includes(key))) {
      throw new RequestError('Invalid request body', 400)
    }
    const action = typeof body.action === 'string' ? body.action : 'validate'
    if (!['validate', 'status', 'list_status', 'connection_status'].includes(action)) {
      throw new RequestError('Invalid action', 400)
    }
    if (action === 'connection_status') return jsonResponse(await connectionStatus(db))
    if (action === 'list_status') {
      ...
      return jsonResponse(await listBrowserResults(db, invoiceIds))
    }
    ...
    const { eligible } = await authorizeInvoiceScope(db, invoiceId)
    ...
    const result = await validateInvoice(db, invoiceId)
