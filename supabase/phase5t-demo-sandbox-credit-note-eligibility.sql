-- ============================================================
-- Phase 5T: Permanent-demo Sandbox credit-note eligibility
-- Apply manually after Phase 5S.
--
-- Keeps the production reported/cleared rule unchanged. The only
-- exception is the exact permanent-demo Trading branch, where the
-- original invoice must have a successful isolated validation attempt.
-- ============================================================
...
DECLARE
  ...
  IF v_original.zatca_status NOT IN ('reported', 'cleared')
     AND NOT (
       ...
       AND v_original.branch_id = '14271653-b404-44bf-9f39-7e9927569c02'::uuid
       AND EXISTS (
         SELECT 1
...
BEGIN
  ...
  IF POSITION(v_existing IN v_definition) = 0 THEN
    RAISE EXCEPTION 'Expected create_partial_credit_note eligibility guard was not found';
  END IF;
