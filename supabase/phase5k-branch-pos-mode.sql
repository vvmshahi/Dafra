-- ============================================================
-- Phase 5K: Branch POS Mode Selection
-- Apply manually after Phase 5C-6 POS touch navigation buttons.
-- ============================================================
--
-- Goals:
--   - Add a branch-level POS mode setting: touch or quick.
--   - Keep branch POS settings updates behind the narrow SECURITY DEFINER RPC.
--   - Preserve existing split-payment and touch-navigation settings.
...
ALTER TABLE public.branches
  ADD COLUMN IF NOT EXISTS pos_mode TEXT;

ALTER TABLE public.branches
  ALTER COLUMN pos_mode SET DEFAULT 'touch';
...
ALTER TABLE public.branches
  ALTER COLUMN pos_mode SET NOT NULL;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'branches_pos_mode_check'
      AND conrelid = 'public.branches'::regclass
  ) THEN
    ALTER TABLE public.branches
      ADD CONSTRAINT branches_pos_mode_check
      CHECK (pos_mode IN ('touch', 'quick'));
  END IF;
END $$;

COMMENT ON COLUMN public.branches.pos_mode IS
  'Controls which POS billing interface a branch uses: touch or quick.';
