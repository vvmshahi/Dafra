...
FOR SELECT TO authenticated USING (
  ...
  AND (public.get_my_role()::TEXT = 'owner' OR branch_id = public.get_my_branch_id())
);
...
BEGIN
  ...
  IF NOT FOUND OR (v_user.role <> 'owner' AND v_user.branch_id IS DISTINCT FROM p_branch_id) THEN RAISE EXCEPTION 'Forbidden' USING ERRCODE='42501'; END IF;
...
RETURNS SETOF public.audit_events LANGUAGE sql STABLE SECURITY DEFINER SET search_path=public SET row_security=off AS $$
  ...
  WHERE a.branch_id=p_branch_id AND a.tenant_id=u.tenant_id AND u.is_active
    AND (u.role::TEXT = 'owner' OR u.branch_id=p_branch_id)
