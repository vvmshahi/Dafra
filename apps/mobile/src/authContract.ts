...
export function validateEmailIdentifier(value: string): string | null {
  const email = value.trim();
  if (!email) return "Email or Branch username is required.";
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email))
...
export function validateBranchUsername(value: string): string | null {
  ...
  if (username.includes("@") || username.length < 3 || username.length > 32)
    return "Use 3–32 lowercase letters, numbers, underscore, or hyphen.";
  if (!USERNAME_ALLOWED_PATTERN.test(username))
    return "Use 3–32 lowercase letters, numbers, underscore, or hyphen.";
  if (
    USERNAME_EDGE_SEPARATOR_PATTERN.test(username) ||
    USERNAME_REPEATED_SEPARATOR_PATTERN.test(username)
  )
    return "Use 3–32 lowercase letters, numbers, underscore, or hyphen.";
...
export function getSignInFormState(
  identifier: string,
  password: string,
  ...
  const identifierError =
    kind === "email"
      ? validateEmailIdentifier(identifier)
      : validateBranchUsername(identifier);
  return {
    kind,
    identifierError,
    canSubmit:
...
export function safeAuthMessage(
  ...
  if (category === "username-not-found")
    return "We could not find that Branch username.";
  if (category === "invalid-password") return "The password is incorrect.";
  if (category === "resolver-unavailable")
    return "Branch username sign-in is temporarily unavailable. Try again shortly.";
  if (category === "network")
    return "Unable to connect. Check your internet connection and try again.";
  if (category === "session-expired")
