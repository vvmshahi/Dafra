...
const api = read('src/mobileApi.ts')
...
test('public production configuration is validated without secret credentials', () => {
  ...
  assert.doesNotMatch(auth + api + envExample, /service[_-]?role|database.password|jwt.signing|production_csid/i)
})
test('production data queries are tenant and Branch scoped', () => {
  for (const table of ["'products'","'customers'","'invoices'"]) assert.match(api, new RegExp(`from\\(${table}\\)`))
...
test('authoritative dashboard, session and register contracts are reused', () => {
  for (const rpc of ['get_dashboard_summary','get_register_session_summary','open_register_session','close_register_session']) assert.match(api, new RegExp(rpc))
})
...
test('barcode resolution is server-scoped and does not download every product', () => {
  const resolver = api.slice(api.indexOf('export async function resolveProductBarcode'))
